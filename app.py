...
def chat():
    ...
    session['name'] = str(theuserid)
    session['room'] = '11'
    name = session.get('name')
    room = session.get('room');
    print("EEEEeEEEEEEEEE %s", name)
    return render_template('message.html', name=name, room=room)


@socketio.on('joined', namespace='/message')
def joined(message):
...
@socketio.on('left', namespace='/message')
def left(message):
...
def login():
	...
	if request.method == 'POST':
		...
		try:
			...
			users = [dict(userid= row[0], useremail=row[1], password=row[7], userfirst=row[3], userlast=row[4], usermiddle=row[5], confirmed=row[10], userrank=row[9]) for row in cur.fetchall()]
...
def newsfeed():
	...
	get_posts = "SELECT user_first, user_last, user_middle, post_description, post_date, post_owner, post_location, post_media,post_id FROM db_hoppa.posts JOIN db_hoppa.hoppers WHERE db_hoppa.hoppers.user_id = db_hoppa.posts.post_owner;"
...
def send_notification(notification, object_data):
	if notification == "join": # Generates notifications on joining a group and sends email
		try:
			...
			description = theusername + " has joined your group: " + group_data['groupname'] + "."
			email = owner_data['owner_email']

			subject = theusername + " has joined " + group_data['groupname']
			join_msg = Message(subject=subject, sender=("Hoppa!", "hoppa.notifications@gmail.com"), bcc=[email], recipients=["hoppa.notifications@gmail.com"])
			join_msg.body = description
			mail.send(join_msg)
			...
			flash("Something went wrong while generating group notifications.")

	elif notification == "attend": # Generates notifications on attending an event and sends email
		try:
			event_data = object_data
			get_owner_info = "SELECT user_id, user_email " \
							 "FROM db_hoppa.hoppers NATURAL JOIN db_hoppa.events " \
							 "WHERE user_id=event_owner AND group_id='%d'" % event_data['event_id']
			cur.execute(get_owner_info)
			owner_info = [dict(owner_id=row[0], owner_email=row[1]) for row in cur.fetchall()]

			description = theusername + " will be attending " + event_data['eventtitle'] + "."
	...
	elif notification == "friend": # Generates notifications on adding a friend and sends email
		try:
			friend_data = object_data
			get_friend_info = "SELECT user_email " \
							  "FROM db_hoppa.hoppers" \
							  "WHERE user_id='%d'" % friend_data['friend_id']
...
def join_group(groupid):
	...
	try:
		...
		send_notification("join", group_data)
...
def attend_event(eventid):
	...
	try:
		...
		send_notification("attend", event_data)
...
def add_friend(friendid):
	...
	try:
		...
		send_notification("friend", friend_data)
	...
	if request.method == 'POST':
		...
		if request.form['buttons'] =='upload':
			...
			try:
				...
				flash("Cannot update your information.")
	return render_template('myprofile.html', user_data=user_data, user_education=user_education)
